...
@section('content')
    ...
    <ul class="menu menu-horizontal gap-1 bg-base-200 rounded-box">
        <li>
            <a href="{{ url('pets/create') }}" class="btn btn-success btn-outline">
        ...
        <li>
            <input class="py-2" id="qsearch" name="qsearch" type="search" placeholder="Search...">
        </li>
                ...
                @foreach ($pets as $pet)
                    <tr class="hover:bg-base-300">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="avatar">
                                    <div class="mask mask-squircle h-12 w-12">
                                        <img src="{{ asset('images/'.$pet->photo) }}" alt="Photo" />
                                    </div>
                                </div>
                                <div>
                                    <div class="font-bold">{{ $pet->name }}</div>
                                    <div class="text-sm opacity-50"> AGE: {{ $pet->age }}</div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            ...
                            {{-- Botón Eliminar --}}
                            <a href="javascript:;"
                                 class="btn btn-outline btn-square btn-error btn-xs btn-delete" data-fullname="{{ $pet->name }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="size-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <script>
        ...
        btnDelete.forEach(element => {
            element.addEventListener('click', function() {
                ...
                textConfirm.textContent = `Are you sure you want to delete the pet ${fullname}?`;
                confirm.showModal();
            });
        });
